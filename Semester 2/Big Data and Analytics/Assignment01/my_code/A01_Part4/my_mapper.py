...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for item in processed_data:
        if item[11] == my_mapper_input_parameters[0]:
            truck_data.append([item[0], item[1], item[4], item[8]])

    truck_data.sort()
    for item in truck_data:
        date_data.add(item[0].split()[0])

    for date in date_data:
        append_string = "universal \t ("
        for item in truck_data:
            if date in item[0]:
                append_string += f"{item[0]} @ {item[1]} @ {item[2]} @ {item[3]} @ "
        append_string = append_string[:-2]
        append_string = append_string.strip()
        append_string += ") \n"
        output_data.append(append_string)

    for item in output_data:
        # universal \t (all dates and locations of a day) \n
