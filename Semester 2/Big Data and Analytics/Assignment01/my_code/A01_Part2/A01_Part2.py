...
def my_main(input_folder, output_file, bike_id):
    processed_data, truck_data, output_data = [], [], []

    files = [input_folder + file for file in os.listdir(input_folder) if
             os.path.isfile(os.path.join(input_folder, file)) and file.endswith('.csv')]
    for file in files:
        with codecs.open(file, 'r') as f:
            for row in f:
                processed_data.append(process_line(row))

    for item in processed_data:
        if item[11] == bike_id:
            truck_data.append([item[0], item[4], item[1], item[8]])

    truck_data.sort()
    i = 0
    while i < len(truck_data) - 1:
        if truck_data[i][3] != truck_data[i+1][1]:
            output_data.append([truck_data[i][2], truck_data[i][3], truck_data[i+1][0], truck_data[i+1][1]])
        i += 1

    with codecs.open(output_file, 'w') as file:
        for item in output_data:
            # By_Truck \t (time_it_was_logged_at_station2, station2_id, time_it_was_logged_at_station3, station3_id) \n
            file.write(f"By_Truck \t ({item[0]}, {item[1]}, {item[2]}, {item[3]}) \n")
