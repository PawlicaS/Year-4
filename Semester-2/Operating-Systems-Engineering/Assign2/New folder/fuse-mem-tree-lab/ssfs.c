...
extern int statit(const char *name,int *psize);
extern int init();
static int do_getattr( const char *path, struct stat *st )
{
	...
	{
	        int size=0;
	        int type = statit(path,&size);
	        printf("after statit %s %d %d\n",path,size,type);
	        if (type ==1)
	        {
...
extern int do_readdir( const char *path, void *buffer, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi );
...
extern int do_read( const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi);
