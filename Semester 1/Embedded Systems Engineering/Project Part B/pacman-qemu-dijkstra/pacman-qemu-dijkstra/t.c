...
//uprintf("rand %d\n",(rand()+1)%5);
     ...
     switch(key){
       case 's':
         if (boundary_check(x-16, y)) {
            x-=16;
            move =1;
         }
         break;
       case 'w':
         if (boundary_check(x, y-16)) {
            y-=16;
            move =1;
         }
         break;
...
uprintf("point\n");
      ...
      if (spriteMove){
         for (int g = 0; g < 3; g++) {
          if (sprites[g].x == x && sprites[g].y == y ){
             if (!dead)
             uprintf("u are dead\n");
             dead=1;
             continue;
          ...
          if (!sprites[g].path){
			...
			*/
                int pos = sprites[g].pathpos; 
                int vertex = sprites[g].pathV[pos];
                sprites[g].x = (vertex % Width) * 16;
                sprites[g].y = (vertex / Width) * 16;
                sprites[g].pathpos=1;
                sprites[g].pathlen=solnlen;
                sprites[g].path =1;
                }
             else{
               uprintf("no soln");
            ...
            if (sprites[g].pathpos == sprites[g].pathlen)
               sprites[g].path=0; // get new path next time
            else
                if (sprites[g].pathpos < sprites[g].pathlen){
