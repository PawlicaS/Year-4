...
int putback(int buf[16][16],int oldstartRow, int oldstartCol){
    ...
    int x,y;
/* TODO */
    for(y=0, j=oldstartRow; j<h+oldstartRow; j++, y++){
      for(x=0, i=oldstartCol; i<w+oldstartCol; i++, x++){
        fb[j*640 + i] = buf[y][x];
      }
    }
}
...
black_point(int startRow, int startCol){
/* TODO */
  int h=16,w=16;
  int i,j;
  int x,y;
  for(y=startRow, j=startCol; j<h+startCol; j++, y++){
      for(x=startCol, i=startRow; i<w+startRow; i++, x++){
        fb[y*640 + x] = 0;
      }
    }
}
